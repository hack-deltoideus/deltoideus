...
const SMOOTHING_WINDOW_ESTIMATES = 12;
...
export class RespiratoryRateEstimator {
	...
	private smoothedEstimates: number[] = [];
	...
	private estimate(): RespiratoryRateEstimate {
		...
		if (fused.bpm !== null) {
			this.smoothedEstimates = [...this.smoothedEstimates, fused.bpm].slice(
				-SMOOTHING_WINDOW_ESTIMATES
			);
		}

		const smoothedBpm =
			this.smoothedEstimates.length > 0 ? median(this.smoothedEstimates) : fused.bpm;
		const confidence = warmingUp ? Math.min(fused.confidence, 0.25) : fused.confidence;

		return {
			breathsPerMinute: smoothedBpm === null ? null : Number(smoothedBpm.toFixed(1)),
...
function detectRPeaks(rawSamples: number[], filteredSamples: number[], sampleRateHz: number): number[] {
	...
	for (let index = 1; index < filteredSamples.length; index += 1) {
		const previousInBlock = qrsAverage[index - 1] > beatAverage[index - 1];
		const inBlock = qrsAverage[index] > beatAverage[index];
...
function estimateQrsRmsByHeartbeatWindow(
	...
	const bestIndex = candidateIndexes.reduce((best, index) =>
		powers[index] > powers[best] ? index : best
	);
	const bpm = (frequencyVector[bestIndex] * 60000) / medianRrMs;
	const averageCandidatePower = mean(candidateIndexes.map((index) => powers[index]));
	const quality = Math.max(
		0,
		Math.min(1, (powers[bestIndex] / Math.max(averageCandidatePower, 1) - 1) / 8)
	);
...
function resampleFeatures(
	...
	for (let index = 0; index < length; index += 1) {
		...
		while (
			...
			features[featureIndex + 1].timeSeconds < time
		) {
			featureIndex += 1;
		}
		...
		const ratio = Math.max(0, Math.min(1, (time - left.timeSeconds) / span));
...
function estimateDominantBpm(
	...
	for (let bpm = minBpm; bpm <= maxBpm; bpm += 0.5) {
		...
		for (let index = 0; index < detrended.length; index += 1) {
			...
			const sample = detrended[index] * window;
			real += sample * Math.cos(angle);
			imaginary -= sample * Math.sin(angle);
		}
	...
	const sorted = powers.toSorted((left, right) => right.power - left.power);
	const peak = sorted[0];
	...
	const quality = Math.max(0, Math.min(1, (peak.power / Math.max(averagePower, 1) - 1) / 8));
...
function fuseEstimates(
	...
	const confidence =
		mean(usable.map((item) => item.estimate.quality)) * 0.65 + agreement * 0.35;
...
function highPassMovingAverage(values: number[], windowSize: number): number[] {
	...
	return values.map((value, index) => value - lowPassed[index]);
}
...
function correctRPeak(values: number[], peakIndex: number): number {
	...
	while (corrected > 0 && values[corrected] < values[corrected - 1]) {
		corrected -= 1;
	}

	while (corrected < values.length - 1 && values[corrected] < values[corrected + 1]) {
		corrected += 1;
	}
...
function downsample(values: number[], sourceRateHz: number, targetRateHz: number): number[] {
	...
	for (let index = 0; index < values.length; index += ratio) {
		result.push(values[Math.floor(index)]);
	}
